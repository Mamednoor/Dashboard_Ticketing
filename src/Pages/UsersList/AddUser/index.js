...
import { Alert, Form, Input, Typography, message, Checkbox } from 'antd'
...
function AddUser() {
	...
	const { isLoading, status } = useSelector((state) => state.addNewUser)
	...
	useEffect(() => {
		if (status === 'success') {
			return setTimeout(() => {
				dispatch(createUserInit()) && history.push('/')
			}, 2000)
		}
